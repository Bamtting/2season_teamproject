$(document).ready(function () {
  ...
  Vue.component("carousel", {
    ...
    methods: {
      ...
      setScrollDirection() {
        if (
          this.$refs.embla.api().selectedScrollSnap() >
          this.$refs.embla.api().previousScrollSnap()
        ) {
      ...
      canScrollPrev() {
        return this.$refs.embla
          ? this.$refs.embla.api().canScrollPrev()
          : false;
      },
  ...
  Vue.component("modal", {
    ...
    methods: {
      fetchVideo() {
        fetch(
          `https://api.themoviedb.org/3/movie/${this.id}/videos?api_key=${userKey}&language=en-US`
        )
          .then((res) => res.json())
          .then((data) => {
            setTimeout(() => {
  ...
  const app = new Vue({
    ...
    methods: {
      ...
      fetchMovieList() {
        this.loading = true;
        fetch(
          `https://api.themoviedb.org/3/list/142235?api_key=${userKey}&language=en-US`
        )
          .then((res) => res.json())
